-- SQL Script to CREATE NEW blog posts in production with author
...
-- Get the first author (or admin user) and first category
WITH author_and_category AS (
  SELECT
    (SELECT id FROM auth.users WHERE email = 'hirendra.vikram@aiborg.ai'
     UNION ALL
     SELECT id FROM auth.users LIMIT 1) as author_id,
    (SELECT id FROM blog_categories LIMIT 1) as category_id
)
...
SELECT
  ...
  'Voice cloning has arrived, and it''s both amazing and terrifying. With just 3 minutes of audio, AI can now create a perfect digital clone of anyone''s voice. Your voice. Your mom''s voice. Even celebrities and politicians. Here''s everything you need to know about this game-changing technology.',
...
SELECT
  ...
  'The clock is ticking. In 5-10 years, quantum computers will break the encryption protecting your bank account, medical records, and private messages. This isn''t sci-fi paranoia - it''s a mathematical certainty that has governments and companies scrambling.',
...
SELECT
  ...
  'Elon Musk''s Neuralink has successfully implanted brain chips in humans. Patients are controlling computers with thoughts, playing chess with their minds, and regaining abilities lost to paralysis. But as brain-computer interfaces go mainstream, we face an unprecedented question: Who owns your thoughts when they become digital data?',
...
SELECT
  ...
  'Last year, 20 million people paid for AI companionship. By 2025, the AI relationship industry will be worth $2 billion. Men are marrying chatbots. Women are leaving real partners for AI. And tech companies are getting rich on human loneliness.',
...
SELECT
  ...
  'China watches 1.4 billion people through 600 million cameras powered by AI. Every face recognized. Every movement tracked. Social credit scores determine who can travel, get loans, or send kids to good schools. This isn''t dystopian fiction - it''s Tuesday in Beijing.',
