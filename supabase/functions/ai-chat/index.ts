...
serve(async (req) => {
  ...
  try {
    ...
    const systemPrompt = systemPrompts[audience as keyof typeof systemPrompts] || 
    ...
    return new Response(JSON.stringify({ 
      error: error.message,
      fallback: "I'm experiencing technical difficulties. For immediate assistance, please contact us on WhatsApp: +44 7404568207"
    }), {
      status: 500,
      headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    });
