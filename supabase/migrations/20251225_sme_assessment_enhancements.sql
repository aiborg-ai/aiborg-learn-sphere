...
CREATE POLICY "Users can view their own roadmap items"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM sme_assessments
      WHERE sme_assessments.id = sme_roadmap_items.assessment_id
      AND sme_assessments.user_id = auth.uid()
    )
  );
...
CREATE POLICY "System can insert roadmap items"
  ...
  WITH CHECK (
    EXISTS (
      SELECT 1 FROM sme_assessments
      WHERE sme_assessments.id = assessment_id
      AND sme_assessments.user_id = auth.uid()
    )
  );
...
CREATE POLICY "Users can view their own roadmap phases"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM sme_assessments
      WHERE sme_assessments.id = sme_roadmap_phases.assessment_id
      AND sme_assessments.user_id = auth.uid()
    )
  );
...
CREATE POLICY "Users can view their own roadmap milestones"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM sme_assessments
      WHERE sme_assessments.id = sme_roadmap_milestones.assessment_id
      AND sme_assessments.user_id = auth.uid()
    )
  );
...
CREATE POLICY "Users can view their own ROI summary"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM sme_assessments
      WHERE sme_assessments.id = sme_roi_summary.assessment_id
      AND sme_assessments.user_id = auth.uid()
    )
  );
...
CREATE POLICY "Users can view their own ROI costs"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM sme_assessments
      WHERE sme_assessments.id = sme_roi_cost_breakdown.assessment_id
      AND sme_assessments.user_id = auth.uid()
    )
  );
...
CREATE POLICY "Users can view their own ROI benefits"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM sme_assessments
      WHERE sme_assessments.id = sme_roi_benefit_breakdown.assessment_id
      AND sme_assessments.user_id = auth.uid()
    )
  );
