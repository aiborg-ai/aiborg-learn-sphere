-- 50 AI Blog Articles - Fixed for Actual Schema (Part 1: Articles 1-2)
...
INSERT INTO blog_categories (name, slug, description, color) VALUES
  ('AI News', 'ai-news', 'Latest news and announcements in artificial intelligence', '#3B82F6'),
  ('Enterprise AI', 'enterprise-ai', 'AI solutions for business and enterprise', '#8B5CF6'),
  ('AI Research', 'ai-research', 'Cutting-edge AI research and breakthroughs', '#EC4899'),
  ('Business AI', 'business-ai', 'AI business impact and ROI', '#10B981'),
  ('AI Development', 'ai-development', 'Tools and techniques for AI development', '#F59E0B')
...
BEGIN
  -- Get category ID
  SELECT id INTO v_category_id FROM blog_categories WHERE slug = 'ai-news';
...
*Stay updated on the latest AI developments with AIBORG. Learn how to leverage AI tools for your business.*',
  ...
  SELECT v_post_id, id FROM blog_tags WHERE slug IN ('openai', 'chatgpt', 'search', 'google', 'technology');
...
BEGIN
  -- Get category ID
  SELECT id INTO v_category_id FROM blog_categories WHERE slug = 'enterprise-ai';
...
*Learn how to implement AI agents in your organization with AIBORG training programs and consulting services.*',
  ...
  -- Add tags
  INSERT INTO blog_post_tags (post_id, tag_id)
  SELECT v_post_id, id FROM blog_tags WHERE slug IN ('microsoft', 'ai-agents', 'automation', 'copilot', 'enterprise');
...
GROUP BY bp.id, bp.title, bc.name, bp.reading_time, bp.published_at
