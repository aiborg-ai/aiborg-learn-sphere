...
export function useVideoNotes(user: User | null, contentId: string, courseId?: number) {
  ...
  const loadNotes = useCallback(async () => {
    ...
    try {
      ...
      logger.error('Error loading notes:', _error);
    }
  }, [user, contentId]);
  ...
  const addNote = useCallback(
    async (currentTime: number) => {
      ...
      try {
        ...
        logger.error('Error adding note:', _error);
        toast({
          title: 'Error',
  ...
  const updateNote = useCallback(
    async (noteId: string, newText: string) => {
      try {
        ...
        logger.error('Error updating note:', _error);
        toast({
          title: 'Error',
  ...
  const deleteNote = useCallback(
    async (noteId: string) => {
      try {
        ...
        logger.error('Error deleting note:', _error);
        toast({
          title: 'Error',
