...
export function useVideoNotes(user: any, contentId: string, courseId?: number) {
  ...
  const addNote = useCallback(async (currentTime: number) => {
  ...
  const updateNote = useCallback(async (noteId: string, newText: string) => {
    try {
      await supabase
        .from('shared_content')
        .update({
