...
export const PersonalizedContent: React.FC<PersonalizedContentProps> = ({ section, children }) => {
  ...
  if (section === 'hero' && config && 'features' in config && config.features) {
    return (
      <Card
        className={`mt-8 p-6 rounded-2xl border animate-fade-in ${getPersonalizedStyles({
          primary:
            'bg-gradient-to-br from-yellow-50 to-orange-50 border-orange-200 dark:from-yellow-950/20 dark:to-orange-950/20 dark:border-orange-800',
          secondary:
            'bg-gradient-to-br from-blue-50 to-purple-50 border-purple-200 dark:from-blue-950/20 dark:to-purple-950/20 dark:border-purple-800',
          professional:
            'bg-gradient-to-br from-slate-50 to-gray-50 border-slate-200 dark:from-slate-950/20 dark:to-gray-950/20 dark:border-slate-800',
          business:
            'bg-gradient-to-br from-emerald-50 to-teal-50 border-emerald-200 dark:from-emerald-950/20 dark:to-teal-950/20 dark:border-emerald-800',
          default: 'bg-card/50 border-primary/20',
        })}`}
      >
        <div className="flex items-center gap-3 mb-4">
          <div
            className={`w-12 h-12 rounded-full flex items-center justify-center ${getPersonalizedStyles(
              {
        ...
        <p className="text-muted-foreground mb-4">
          {getPersonalizedContent({
            primary:
              "Get ready for an amazing adventure with AI! We'll learn through fun games, colorful activities, and exciting challenges designed just for you.",
            secondary:
              'Dive into cutting-edge AI technology with hands-on projects, career guidance, and skills that will prepare you for the future.',
            professional:
              'Access industry-leading AI training with certifications, expert insights, and practical applications for your career growth.',
            business:
              "Unlock AI's potential for your organization with enterprise solutions, team training, and measurable business impact.",
        ...
        <div className="mt-4 pt-3 border-t border-border/50">
          <div className="flex items-center justify-between">
            <Badge variant="secondary" className="text-xs">
              {config && 'language' in config ? config.language.level.toUpperCase() : 'GENERAL'}{' '}
              LEVEL
            </Badge>
