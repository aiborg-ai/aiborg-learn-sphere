 ...
 *   console.warn('Need at least 60 days of historical data');
 * }
 *
 ...
 *
 * // Access forecast
 * console.log(state.revenue.data?.forecast);
 * console.log(state.revenue.data?.confidence); // R² value
 * console.log(state.revenue.data?.quality); // 'excellent', 'good', 'fair', 'poor'
 * ```
 *
...
export function usePredictiveAnalytics(): UsePredictiveAnalyticsReturn {
   ...
   */
  const hasInsufficientData = useCallback((type: 'revenue' | 'userGrowth' | 'enrollments'): boolean => {
   ...
   */
  const getQualityDescription = useCallback((type: 'revenue' | 'userGrowth' | 'enrollments'): string | null => {
    const result = state[type].data;
   ...
   */
  const forecastRevenue = useCallback(async (historicalData: RevenueData[], days: 30 | 60 | 90): Promise<void> => {
    try {
      setState(prev => ({
        ...prev,
        revenue: {
   ...
   */
  const forecastUserGrowth = useCallback(async (historicalData: UserData[], days: 30 | 60 | 90): Promise<void> => {
    try {
      setState(prev => ({
        ...prev,
        userGrowth: {
   ...
   */
  const forecastEnrollments = useCallback(async (historicalData: EnrollmentData[], days: 30 | 60 | 90): Promise<void> => {
    try {
      setState(prev => ({
        ...prev,
        enrollments: {
