...
export const useCoursesOptimized = (options: UseCoursesOptions = {}) => {
  ...
  const fetchCourses = useCallback(async () => {
    try {
      ...
      if (includeInstructor && processedCourses.length > 0) {
        const instructorIds = [...new Set(processedCourses
        ...
        if (instructorIds.length > 0) {
          ...
          if (instructorsData) {
            const instructorsMap = new Map(
              instructorsData.map((i: any) => [i.id, {
                id: i.id,
  ...
  const getCoursesByCategory = useCallback((categoryName: string) => {
    return courses.filter(course => course.category === categoryName);
  }, [courses]);

  const getCoursesByLevel = useCallback((levelName: string) => {
