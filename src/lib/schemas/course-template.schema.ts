...
export const CourseTemplateSchema = z.object({
  ...
  course_materials: z.array(CourseMaterialSchema)
    .optional()
    .refine(
      (materials) => {
  ...
  min_students: z.number()
    .int()
    .positive('Minimum students must be positive')
    .optional(),
  ...
  (data) => {
    ...
    if (data.enrollment_deadline && data.start_date) {
      ...
      if (data.start_date === 'Flexible' || data.start_date === 'Coming Soon' || data.start_date === 'TBD') {
        return true;
      }
      if (data.enrollment_deadline === 'Flexible' || data.enrollment_deadline === 'Coming Soon' || data.enrollment_deadline === 'TBD') {
        return true;
      }
      // Compare dates
