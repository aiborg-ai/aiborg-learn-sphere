...
export const EventTemplateSchema = z.object({
  ...
  what_to_bring: z.array(z.string())
    .max(10, 'Maximum 10 items in what to bring list')
    .optional(),
  ...
  target_audience: z.array(z.string())
    .max(10, 'Maximum 10 target audience groups')
    .optional(),
  ...
  sponsors: z.array(SponsorSchema)
    .max(20, 'Maximum 20 sponsors allowed')
    .optional(),

  contact_info: ContactInfoSchema.optional(),

  // Optional metadata fields
  min_attendees: z.number()
    .int()
    .positive('Minimum attendees must be positive')
    .optional(),
  ...
  (data) => {
    ...
    if (data.registration_deadline && data.date) {
      ...
      if (data.registration_deadline === 'Flexible' || data.registration_deadline === 'Coming Soon' || data.registration_deadline === 'TBD') {
        return true;
      }
      // Compare dates
...
export function checkEventDuplicates(events: EventTemplate[]): {
  ...
  events.forEach((event, index) => {
    ...
    if (event.location.toLowerCase() !== 'online' && !event.location.toLowerCase().includes('virtual')) {
